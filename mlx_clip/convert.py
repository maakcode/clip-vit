...
def convert(
    ...
    """
    Download and Convert (OpenAI) CLIP weights to MLX.

    Args:
        hf_repo (str): Hugging Face repository name.
        mlx_path (str): Path to save the MLX model.
        dtype (str): The data type to save the converted model.
        force_download (bool): Force download the model from Hugging Face.
    """
    torch_path = get_model_path(model_name, force_download)
    ...
    mlx_weights = {k: torch_to_mx(v, dtype=dtype) for k, v in torch_weights.items()}
    print("[INFO] Saving")
